...
FILES_TO_PROCESS = [
    {'input': 'GP_Quality.json', 'sort_name': 'quality'},
    {'input': 'GP_LastUpdate.json', 'sort_name': 'pubdate'}
]

def format_category(cat_raw):
    if not cat_raw:
        return "Game"
    words = cat_raw.split('-')
    ...
    for g in raw_data:
        ...
        if safe_url != "" and safe_url not in seen_urls:
            ...
            if 'thumb' in g:
                ...
                cat = format_category(g.get('category', ''))
                thumb = g.get('banner_image') or g.get('image', '')
